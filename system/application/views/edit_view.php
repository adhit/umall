...
<?php include "includes/header.php"; ?>
    ...
    <div class="container">
	<form action="<?php echo site_url()."/item/edit/".$item['itemID']; ?>" method="post" enctype="multipart/form-data">
	...
	<?php } ?>
    <h1 class="content-header">Edit an item </h1>
   	<div class="row">
        ...
        <div class="span6 offset2">
            <div class="control-group<?php if(isset($msg['name_error'])) echo " error"; ?>">
              ...
              <div class="controls">
				...
				else if(!$this->input->post('filled')&&isset($item['name'])) echo "value=\"".$item['name']."\""; 
				?>>
        ...
        <div class="control-group">
              ...
              <div class="controls">
                <textarea id="input02" class="input-xlarge" name="description" rows=4 placeholder=" ... and your item description here"><?php if($this->input->post('name')) echo $this->input->post('description'); 
				else if(!$this->input->post('filled')&&isset($item['description'])) echo $item['description']; ?></textarea>
			  </div>
        </div>
        <hr>
        <div class="control-group">
              ...
              <div class="controls">
				<input type="hidden" name="type" value="<?php if(isset($item['type'])) echo $item['type']; ?>">
                <select class="input-xlarge select-type" id="input03" disabled="disabled">
                  <option value="fixed" selected>Fixed Price</option>
                  <option value="bid" <?php if(isset($item['type'])&&$item['type']=='bid') echo "selected"; ?>>Negotiable</option>
				</select>
                &nbsp;for  &nbsp;
                <select class="input-xlarge select-expiry" name="exp" id="input03">
					<option value="same" <?php if($this->input->post('exp')=="same") echo "selected";?>>No change</option>
        ...
        <div class="control-group<?php if(isset($msg['price_error'])) echo " error"; ?>">
              ...
              <div class="controls">
				...
				<?php if($this->input->post('price')) echo "value=\"".$this->input->post('price')."\""; 
					else if(!$this->input->post('filled')&&isset($item['price'])) echo "value=\"".$item['price']."\"";?>></div>
        ...
        <div class="control-group<?php if(isset($msg['image_error'])) echo " error"; ?>">
          ...
          <div class="controls">
			<span class="help-block-image">Use existing image:</span>
			<br><img src="<?php echo base_url()."/pictures/".$item['image']; ?>" style="padding:px;" width=200>
			<br><br>
			<span class="help-block-image">or replace with the new one:</span>
			<br>
        ...
        <a href="<?php echo site_url().$prev; ?>" class="btn btn-large">Cancel</a> <button type="submit" class="btn btn-primary btn-large pull-right">Done editing!</button>
        </div>
      </div>
        ...
        <div class="control-group">
          ...
          <div class="controls">
			...
			<?php
				...
				$the_tags=$init_tags;
				if($this->input->post('tags')) $the_tags=$this->input->post('tags');
				$temp_count=0;
				...
				foreach($tags as $val) {
					...
					if($temp%2==0) {
						...
						if($the_tags&&in_array($val['tagID'],$the_tags)) echo "checked";
