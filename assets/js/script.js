$(document).ready(function() {
	...
	if(hash){
			$(hash).css('background','#e9ffd6').animate({ backgroundColor: "#fff"}, 2000);
			$('html, body').animate({ scrollTop: $(hash).offset().top-100/*100px space from the header id*/ });
		}
	...
	viewbidding.click(function(){
		...
		if($this.hasClass('hid')){
			$this.children('span').html('Hide history '); 
			$this.removeClass('hid');
			btable.fadeIn();
		}
		else{
			$(this).children('span').html('Show history ');
			$this.addClass('hid'); 
			btable.fadeOut();
		}
	})	
